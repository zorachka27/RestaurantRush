...
function RestaurantService:Start(dataServiceRef, remoteTable, kitchenServiceRef)
    ...
    if remotes.RequestAction then
        remotes.RequestAction.OnServerEvent:Connect(function(player, action, payload)
            ...
            elseif action == "HireWorker" and workerService then
                local workerName = payload
                if type(workerName) ~= "string" then
                    workerName = tostring(workerName)
                end
                local success, message = workerService:HireWorker(player, workerName)
                if success then
                    state = dataService:GetPlayerState(player)
                else
                    warn(message)
                end
            elseif action == "UpgradeWorker" and workerService then
                ...
                local success, message = workerService:UpgradeWorker(player, workerName)
                if success then
                    state = dataService:GetPlayerState(player)
                else
                    warn(message)
